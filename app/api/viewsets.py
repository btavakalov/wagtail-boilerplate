...
class ResponseWithRetrieveSerializerMixin:
    ...
    def response(self, instance, status, headers=None):
        instance = self.get_queryset().get(id=instance.id)  # We do that to apply annotations and stuff from get_queryset()
...
class DefaultCreateModelViewSet(
...
class DefaultCreateReadModelViewSet(
    DefaultCreateModelMixin,
    mixins.RetrieveModelMixin,
    mixins.ListModelMixin,
    ResponseWithRetrieveSerializerMixin,
